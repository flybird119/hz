$(document).ready(function () {
    ...
    $(".btn_blue").on("touchend ", function () {
        ...
        if (man.stat.shield === 1 ) {//判断进入屏蔽菜单
            ...
            if(man.stat.nc_flag===1){//判断输入类型，1为设备代码0为设备号
                if(z>3){
                    return;
                }
                man.res.shield_code += man.switch_shiled_inp(x,z);
                if(man.res.shield_code.length>2) {
                    man.res.shield_code = man.res.shield_code.slice(-1);
                }
                ...
                if(man.res.shield_num.length>6){
                    man.res.shield_num = (man.res.shield_num+man.switch_shiled_inp(x,z)).slice(-1);
                }
                man.draw_shield(man.res.shield_num,man.res.shield_code);
            }
        }
    ...
    $("#blue_11").on("touchend ", function () {
        if(man.stat.reset===1 ||man.stat.shield===1 || man.stat.self_check===1||man.stat.his_rec===1 || man.stat.set_start_mod===1 || man.stat.pwd ===1){
            return;
        }
    ...
    $("#blue_14").on("touchend ", function () {
        ...
        if(man.stat.shield===1){//屏蔽菜单下
            man.stat.nc_flag=Math.abs(man.stat.nc_flag-1);
            man.draw_shield(man.res.shield_num,man.res.shield_code);
        }
    });
    ...
    $("#blue_21").on("touchend ", function () {
        ...
        if(man.scrn.self_check.menu_show===1 || man.scrn.rec_check.menu_show===1 || man.stat.pwd===1){
            return;
        }
    ...
    $("#blue_22").on("touchend ", function () {
        ...
        if(man.ck_stat_1()){return;}
        if(man.scrn.self_check.menu_show===1 || man.stat.shield===1 || man.scrn.rec_check.menu_show===1|| man.stat.pwd ===1){
            return;
        }
        if(man.scrn.rec_check.his_rec_list===1){
            man.scrn.rec_check.his_rec_list=0;
    ...
    $("#blue_34").on("touchend ", function () {
        ...
        if(man.scrn.self_check.menu_show===1 || man.stat.shield===1 || man.scrn.rec_check.menu_show===1){
            return;
        }
        man.draw_pwd_box();
        man.stat.pwd=1;
    ...
    $("#blue_42").on("touchend ", function () {
        ...
        if(man.scrn.self_check.menu_show===1 || man.stat.shield===1 || man.stat.pwd===1 || man.stat.set_start_mod===1){
            return;
        }
        if(man.stat.shield===1){
            return;
...
Robot.prototype.ck_stat_1=function () {
    return this.stat.set_start_mod===1 || this.stat.shield===1 || this.stat.shield_cancel===1||this.stat.his_rec===1;
};
...
Robot.prototype.draw_shield = function (num,code) {
    ...
    ctx.strokeRect(w / 6 +tip.length*12+5, h / 2 - 6, 60, 20);
    (man.stat.nc_flag==0)?ctx.strokeStyle="#7a84fb":ctx.strokeStyle="#fff";
    ctx.strokeRect(w / 6 +tip.length*25, h / 2 - 6, 90, 20);
    ctx.fillText(tip,w/6-10,h/2+10);
    ctx.fillText(num, w/6+tip.length*12+10, h / 2+10);
    ctx.fillText("号",w/6+tip.length*22,h/2+10);
    ctx.font = "bold 12px 微软雅黑";
    ctx.fillText(code+this.switch_shiled_code(code), w / 6 +tip.length*25+5, h / 2+10);
    ctx.fillStyle=old_style;
    ctx.font=old_font;
    ctx.strokeStyle = old_stroke;
};
Robot.prototype.draw_shield_srn = function (num,code) {
    ...
    ctx.fillText(this.switch_shiled_code(code).slice(1), w / 2 , 40*2-15);
    ctx.fillStyle=old_style;
    ctx.font=old_font;
    ctx.strokeStyle = old_stroke;
};
