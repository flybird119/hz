{foreach $tf as $row}
    <div class="row hidden" id="{'q'|cat:{$row@iteration+$tf_start}}">
        <!--进度开始-->
        <div class="container">
            <div class="pull-right counter">
                <span> 进度：</span>
                <span>{$row@iteration+$tf_start} /</span>
                <span>{$count}</span>
            </div>
        </div>
        <!--ban开始-->
        <div id="{'qid'|cat:{$row@iteration+$tf_start}}" class="hidden">{$row[0]}</div>
        <div id="{'RA'|cat:{$row@iteration+$tf_start}}" class="hidden">{$row[4]|md4x}</div>
        <div id="{'SCORE'|cat:{$row@iteration+$tf_start}}" class="hidden">{$exam["tfScore"]}</div>

        <div class="container">
            <div class=" banner_question">
                <div class="row banner_title">
                    <div class="col-xs-4 banner_top_left "></div>
                    <div id="{'type'|cat:{$row@iteration+$tf_start}}" class="hidden">{$row[1]}</div>
                    ...
                    <div class="col-xs-12">
                        <div class="items">
                            ...
                            {foreach $sels as $i}
                                <div class="radio">
                                    <label>
                                        <input type="radio" name="{'r'|cat:{$row@iteration+$tf_start}}"
                                               value="{$i@iteration}"/>
